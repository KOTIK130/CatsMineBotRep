...
from config import users_collection, db, MAINTENANCE_MODE, OWNER_ID
...
async def confirm_clear_db(message: Message):
    ...
    if message.from_user.id != OWNER_ID:  # Только главный админ
...
async def clear_database(message: Message):
    if not is_admin(message.from_user.id) or message.from_user.id != OWNER_ID:
        return
