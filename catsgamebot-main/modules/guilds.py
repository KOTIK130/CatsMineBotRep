...
async def guild_menu(message: Message):
    ...
    if guild_id:
        ...
        if guild:
            ...
            text = (
                f"⛵ <b>Добро пожаловать в гильдии!</b>\n\n"
                f"🏴‍☠️ Ваша гильдия: <b>{guild['name']}</b>\n"
...
async def my_guild(message: Message):
    ...
    if not guild_id:
        await message.answer("❌ Вы не состоите в гильдии!", reply_markup=guild_main_keyboard())
        return

    guild = await guilds_collection.find_one({"_id": guild_id})
    if not guild:
        await message.answer("❌ Гильдия не найдена!", reply_markup=guild_main_keyboard())
        return
    ...
    if not member_info:
        await message.answer("❌ Вы не найдены в списке участников!", reply_markup=guild_main_keyboard())
        return
    ...
    keyboard.inline_keyboard.extend([
        ...
        [InlineKeyboardButton(text="🚪 Покинуть гильдию", callback_data="guild_leave")]
    ])
...
async def open_guild_chat(callback: CallbackQuery):
    ...
    try:
        ...
        logging.error(f"Ошибка при открытии гильд-чата: {e}")
        await callback.answer("❌ Произошла ошибка при открытии чата")

@router.message(F.text == "🔍 Найти гильдию")
async def find_guild(message: Message):
    ...
    if not guilds:
        await message.answer(
            "🔍 <b>Поиск гильдий</b>\n\n"
            "❌ Открытых гильдий не найдено.\n"
            "Создайте свою собственную гильдию!",
            reply_markup=guild_main_keyboard(),
            parse_mode="HTML"
        )
        return

    text = "🔍 <b>Открытые гильдии:</b>\n\n"
    keyboard = InlineKeyboardMarkup(inline_keyboard=[])
...
@router.message(F.text == "🏗️ Создать гильдию")
async def create_guild_start(message: Message, state: FSMContext):
    user_id = message.from_user.id
    user = await users_collection.find_one({"user_id": user_id})
    
    if user.get("guild_id"):
        await message.answer("❌ Вы уже состоите в гильдии!", reply_markup=guild_main_keyboard())
        return

    if user.get("money", 0) < 50000:
        await message.answer(
            "❌ <b>Недостаточно средств!</b>\n\n"
            "💰 Для создания гильдии нужно: <b>50,000$</b>\n"
    ...
    await message.answer(
        ...
        "⛵ Введите название вашей гильдии:\n"
        "(от 3 до 30 символов)",
        parse_mode="HTML"
    )
...
async def create_guild_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "📝 Теперь введите описание гильдии:\n"
        "(до 200 символов)",
        parse_mode="HTML"
    )
